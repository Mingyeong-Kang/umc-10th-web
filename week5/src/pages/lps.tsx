...
function SkeletonCard() {
  return <div className="animate-pulse bg-gray-700 rounded-xl aspect-square" />;
}

function LpCard({ lp, onClick }: { lp: LpItem; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="text-gray-300 text-xs mt-1">{uploadDate}</p>
        <p className="text-pink-400 text-xs mt-1">❤️ {lp.likes.length}</p>
      </div>
    </div>
  );
}

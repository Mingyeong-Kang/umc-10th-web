...
export const signUp = async (payload: SignUpRequest) => {
  const { data } = await axiosInstance.post<ServerResponse<unknown>>(
    "/auth/signup",
    payload
  );
  return data.data;
...
export const updateMyInfo = async (payload: UpdateMyInfoRequest) => {
  const { data } = await axiosInstance.patch<ServerResponse<MyInfoData>>(
    "/users",
    payload
  );
  return data.data;
